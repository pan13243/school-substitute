  ...
  modal.innerHTML = `
    <div style="background:#fff; border-radius:12px; max-width:420px; width:100%; box-shadow:0 20px 60px rgba(0,0,0,0.3);">
      ...
      <div style=\"padding:20px;\">
        ...
        <div class=\"form-group\">
          <label>确认新密码 *</label>
          <input type=\"password\" id=\"reset-confirm-principal-pwd\
        return r;
    }
uerySelector('#reset-principal-pwd').value;
...
function renderTeacherSubTT(teacherName) {
  ...
  (function(){
    ...
    function buildTT(n){
      var r = {};
      var SUBS = ["语文","数学","英语","体育","音乐","美术"];
      var teacher = "演示教师";
      D_DAYS.forEach(function(d){
        r[d] = {};
        var di = D_DAYS.indexOf(d);
        for (var i = 0; i < 3; i++) {
          var cIdx = (di + i) % D_CLS.length;
          var cls = D_CLS[cIdx];
          var s = SUBS[(di + i) % SUBS.length];
          var p = i + 1;
          if (!r[d][cls]) r[d][cls] = [];
          r[d][cls].push({period:p, subject:s, teacher:teacher});
        }
      });
      return r;
    }
    ...
    window.renderTeacherSubTT = function(name){
      ...
      if (window.schoolName === '演示学校' || location.hostname === 'school-kp6nyd7g.pages.dev') {return '<div style="margin-bottom:6px;font-size:12px;color:#92400E;background:#FEF3C7;padding:4px 8px;border-radius:4px;border:1px solid #F59E0B;">🎓 演示课表（模拟数据）</div>' + html;
      }
...
function getTeacherConflict(t, dow, period, leaveDate) {
  ...
  // 尝试两种键格式: normDay 后(周三)和原始(dow=星期三)
  const dayData = scheduleData.timetable?.[targetDow] || (dow !== targetDow ? scheduleData.timetable?.[dow] : null);
  if (dayData) {
    for (const [cls, slots] of Object.entries(dayData)) {
