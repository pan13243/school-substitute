...
const startMarker = `function parseAfterSchoolWorkbook(wb) {`;
const endMarker = `  return { sheet: mainSheet, days, slots, classes: classCols.map(c=>c.name) };\n}`;

const startIdx = s.indexOf(startMarker);
if (startIdx < 0) { console.log('❌ 没找到函数头'); process.exit(1); }
const endIdx = s.indexOf(endMarker, startIdx);
if (endIdx < 0) { console.log('❌ 没找到函数尾'); process.exit(1); }
const endIdx2 = endIdx + endMarker.length;

const oldFn = s.slice(startIdx, endIdx2);
console.log('原函数长度:', oldFn.length);
...
s = s.slice(0, startIdx) + newFn + s.slice(endIdx2);
fs.writeFileSync(p, s, 'utf8');
console.log('✅ 重写成功，新文件大小:', s.length);