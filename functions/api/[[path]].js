    ...
    for (const [slotKey, slot] of Object.entries(teacherSlots)) {
      ...
      if (substitute) {
        results.push({
          ...
          makeupParity: leave.makeupParity || null,  // 同步补课周次
          status: 'arranged'
        });
...
async function handleLeavesPost(request, env) {
  const body = await request.json().catch(() => ({}));
  const { teacherName, leaveDate, dayOfWeek, period, reason, leaveType, needSubstitute, duration, makeupDay, makeupParity } = body;
  ...
  // 补课字段(周末请假时记录补周几和单/双周,用于列表显示"六补五(双周)"格式)
  if (makeupDay) leave.makeupDay = makeupDay;
  if (makeupParity) leave.makeupParity = makeupParity;
...
async function handleActivate(request, env) {
  ...
  // 1. 建 KV namespace
  console.log('[handleActivate] start', { code, schoolName, schoolId });
  ...
  if (!kvData.success) {
    console.log('[handleActivate] KV create FAILED', kvData.errors);
    return json({ success: false, message: '创建 KV 失败: ' + ((kvData.errors && kvData.errors[0] && kvData.errors[0].message) || '') }, 500, corsHeaders);
  }
  const kvId = kvData.result.id;
  console.log('[handleActivate] KV created', kvId);

  // 2. 写 schoolMeta（失败时回滚 KV）
  ...
  try {
    const metaResp = await cfFetch('/client/v4/accounts/' + CF_ACCOUNT_ID + '/storage/kv/namespaces/' + kvId + '/values/schoolMeta', { method: 'PUT', body: JSON.stringify(schoolMeta) });
    if (!metaResp.ok) {
      const body = await metaResp.text();
      console.log('[handleActivate] schoolMeta write FAILED', metaResp.status, body.slice(0, 200));
      await cfFetch('/client/v4/accounts/' + CF_ACCOUNT_ID + '/storage/kv/namespaces/' + kvId, { method: 'DELETE' });
      console.log('[handleActivate] KV rolled back after schoolMeta write failure');
      return json({ success: false, message: '写入学校配置失败（已清理 KV）' }, 500, corsHeaders);
    }
  } catch (e) {
    console.log('[handleActivate] schoolMeta write EXCEPTION', e.message);
    await cfFetch('/client/v4/accounts/' + CF_ACCOUNT_ID + '/storage/kv/namespaces/' + kvId, { method: 'DELETE' });
    return json({ success: false, message: '写入学校配置异常（已清理 KV）' }, 500, corsHeaders);
  }

  // 3. 建 Pages 项目（失败时回滚 KV）
  ...
  try {
    ...
    pagesData = await createPagesResp.json();
  } catch (e) {
    console.log('[handleActivate] Pages create EXCEPTION', e.message);
    await cfFetch('/client/v4/accounts/' + CF_ACCOUNT_ID + '/storage/kv/namespaces/' + kvId, { method: 'DELETE' });
    console.log('[handleActivate] KV rolled back after Pages create exception');
    return json({ success: false, message: '创建 Pages 异常（已清理 KV）' }, 500, corsHeaders);
  }
  if (!pagesData.success) {
    const err = (pagesData.errors && pagesData.errors[0] && pagesData.errors[0].message) || '';
    console.log('[handleActivate] Pages create FAILED', pagesData.errors);
    await cfFetch('/client/v4/accounts/' + CF_ACCOUNT_ID + '/storage/kv/namespaces/' + kvId, { method: 'DELETE' });
    console.log('[handleActivate] KV rolled back after Pages create failure');
    if (err.includes('already')) return json({ success: false, message: '该校已激活，请勿重复提交' }, 400, corsHeaders);
    return json({ success: false, message: '创建学校失败: ' + err + '（已清理 KV）' }, 500, corsHeaders);
  }
  console.log('[handleActivate] Pages created', schoolId);

  // 4. 主动触发首次部署（fire-and-forget，用户访问时已可访问）
  cfFetch('/client/v4/accounts/' + CF_ACCOUNT_ID + '/pages/projects/' + schoolId + '/deployments', {
    method: 'POST', body: JSON.stringify({}),
  }).then(function(r) {
    return r.json().then(function(j) { console.log('[handleActivate] initial deploy', r.status, j.result && j.result.id || j); });
  }).catch(function(e) { console.log('[handleActivate] initial deploy EXCEPTION', e.message); });
