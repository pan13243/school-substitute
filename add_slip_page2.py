...
# 步骤 1-3 已完成，只做步骤4
s = li("// 检查并推送新安排的代课任务（教师端）")
print('Anchor found at line', s+1)

# 检查步骤1-3是否已完成
if "    slip: '请假条管理'," in lines[li("settings: '通知设置'")]:
    print('Step 1 already done')
else:
    s2 = li("settings: '通知设置'")
    lines.insert(s2, "    slip: '请假条管理',\n")
    print('1. pageTitles OK')

if "data-page=\"slip\"" in lines[li("onclick=\"switchPage('import')\">")] or any("请假条管理" in l for l in lines):
    print('Step 2 already done')
else:
    s2 = li("onclick=\"switchPage('import')\">📤 导入课表</button>")
    lines.insert(s2+1, "          <button class=\"nav-btn\" data-page=\"slip\" onclick=\"switchPage('slip')\">📄 请假条管理</button>\n")
    print('2. Sidebar OK')

if "renderSlipPage(area)" in lines[li("else if (page === 'settings') renderSettingsPage(area);")]:
    print('Step 3 already done')
else:
    s2 = li("else if (page === 'settings') renderSettingsPage(area);")
    lines.insert(s2, "  else if (page === 'slip')    renderSlipPage(area);\n")
